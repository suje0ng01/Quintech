    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http
                ...
                .authorizeHttpRequests(auth -> auth
                        .requestMatchers("/", "/login", "/register", "/api/user/register", "/success", "/auth/login", "/oauth2/**","/auth/oauth/google/**","/api/user/**","/auth/**","/check-in/**","/api/password/**").permitAll() // ✅ 로그인, 회원가입은 모두 허용
