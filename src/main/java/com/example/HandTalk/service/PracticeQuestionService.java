...
public class PracticeQuestionService {
    ...
    public List<PracticeQuestionDto> getConsonantQuestions() {
        ...
        return consonants.stream()
                .map(c -> new PracticeQuestionDto(ContentType.CONSONANT, c, firebaseVideoMapper.getVideoUrl(ContentType.CONSONANT, c)))
    ...
    public List<PracticeQuestionDto> getVowelQuestions() {
        ...
        return vowels.stream()
                .map(v -> new PracticeQuestionDto(ContentType.VOWEL, v, firebaseVideoMapper.getVideoUrl(ContentType.VOWEL, v)))
                .collect(Collectors.toList());
    }
    ...
    public List<PracticeQuestionDto> getWordQuestions(String topic) {
        ...
        return words.stream()
                .map(word -> new PracticeQuestionDto(ContentType.WORD, word, firebaseVideoMapper.getVideoUrl(ContentType.WORD, word)))
