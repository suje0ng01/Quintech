import 'package:flutter/material.dart';
import 'package:provider/provider.dart'; //LoginState 사용을 위한 provider

import '../mock_main/mock_main_page.dart'; // TODO : 테스트용 메인 페이지 >> 추후 삭제
import '../mock_main/login_state.dart'; // 로그인 상태
import 'setting_info.dart';
import 'setting_faq.dart';
import 'setting_member.dart';


//설정 페이지
class SettingsPage extends StatefulWidget {
  const SettingsPage({super.key});

  @override
  State<SettingsPage> createState() => _SettingsPageState();
}

class _SettingsPageState extends State<SettingsPage> {
  @override
  Widget build(BuildContext context) {
    final isLoggedIn = Provider.of<LoginState>(context).isLoggedIn;

    return Scaffold(
      appBar: AppBar(
        backgroundColor: Colors.yellow[600],
        title: const Text(
          '설정',
          style: TextStyle(color: Colors.black, fontWeight: FontWeight.bold),
        ),
        centerTitle: true,
        leading: IconButton(
          icon: const Icon(Icons.home, color: Colors.black),
          onPressed: () {
            Navigator.push(context, MaterialPageRoute(builder: (_) => const MockMainPage())); // TODO: 임시 홈 이동 > 홈 화면으로 이동하는 기능 추가
          },
        ),
        actions: [
          IconButton(
            icon: isLoggedIn
                ? const CircleAvatar(
                    backgroundImage: NetworkImage('https://i.pravatar.cc/150?img=50'),
                  )
                : const Icon(Icons.account_circle, size: 36, color: Colors.black), // 로그아웃 상태일 때 아이콘 변경
            onPressed: () {
              // TODO: 프로필 화면으로 이동하는 기능 추가
            },
          ),
          const SizedBox(width: 10),
        ],
      ),
      body: ListView(
        children: [
          if (isLoggedIn) ...[
            _buildListTile(
              '로그아웃',
              Icons.logout,
              () {
                final loginState = Provider.of<LoginState>(context, listen: false);
                loginState.logOut();
              },
            ),
            _buildListTile('회원 정보', Icons.person, () {
              Navigator.push(
                context,
                MaterialPageRoute(builder: (context) => const MemberInfoPage()),
              );
            }),
          ] else ...[
            _buildListTile('로그인/회원가입', Icons.login, () {
              final loginState = Provider.of<LoginState>(context, listen: false);
              loginState.logIn();
            }),
          ],
          const Divider(),
          _buildListTile('공지사항', Icons.announcement, () {
            Navigator.push(
              context,
              MaterialPageRoute(builder: (context) => const NoticePage()),  //공지사항 페이지로 이동동
            );
          }),
          _buildListTile('FAQ', Icons.help, () {
            Navigator.push(
            context,
            MaterialPageRoute(builder: (context) => const FAQPage()), // FAQ 페이지로 이동동
            );
          }),
        ],
      ),
    );
  }

  Widget _buildListTile(String title, IconData icon, VoidCallback onTap) {
    return ListTile(
      leading: Icon(icon, color: Colors.black),
      title: Text(title),
      trailing: const Icon(Icons.arrow_forward_ios, size: 16),
      onTap: onTap,
    );
  }
}